...
package org.opensingular.form.support;
...
@Parameterized.UseParametersRunnerFactory(TestFormSupport.FactoryRunnerParameteziedWithSpring.class)
@ActiveProfiles(resolver = TestFormSupport.ParameterizedFlowProfileResolver.class)
public abstract class TestFormSupport {

    @Parameterized.Parameter(0)
    public FormTestConfig formTestConfig;
    ...
    public static void configApplicationContext(ApplicationContext applicationContext) {
        ...
        try {
            myBeanRef = applicationContext.getBean(TestFormSupport.MyBean.class);
        ...
        if (myBeanRef == null) {
            ...
            ((ConfigurableApplicationContext) applicationContext).getBeanFactory().registerSingleton(
                    TestFormSupport.MyBean.class.getName(), myBeanRef);
        }
    }

    @Parameterized.Parameters(name = "{index}: ({0})")
    public static Collection<FormTestConfig> data() {
        SingularPropertiesImpl.get().reloadAndOverrideWith(ClassLoader.getSystemClassLoader().getResource(
                "singular-mssql.properties"));

        List<FormTestConfig> executionParams = new ArrayList<>();
        executionParams.add(new FormTestConfig("mssql", "singular-mssql.properties"));
        executionParams.add(new FormTestConfig("oracle", "singular-ora.properties"));
    ...
    private static class RunnerParametersWithSpring2 extends SpringJUnit4ClassRunner {
        ...
        private final FormTestConfig formTestConfig;
        ...
        public RunnerParametersWithSpring2(TestWithParameters test) throws InitializationError {
            ...
            formTestConfig = (FormTestConfig) test.getParameters().get(0);
        }

        private static Class<?> prepare(TestWithParameters test) {
            ParameterizedFlowProfileResolver.currentProfile = ((FormTestConfig) test.getParameters().get(0)).getSpringProfile();
            return test.getTestClass().getJavaClass();
        }

        @Override
        protected Object createTest() throws Exception {
            SingularPropertiesImpl.get().reloadAndOverrideWith(ClassLoader.getSystemClassLoader().getResource(formTestConfig.getBdProperties()));
        ...
        public void run(RunNotifier notifier) {
            ParameterizedFlowProfileResolver.currentProfile = formTestConfig.getSpringProfile();
            super.run(notifier);
            ParameterizedFlowProfileResolver.currentProfile = null;
        }

(function () {
    ...
    function getBootstrapTimePickerWidget() {
        return $('.bootstrap-timepicker-widget');
    }
    ...
    function removeExtraTimepickerWidget() {
        if (getBootstrapTimePickerWidget().size() > 1) {
            getBootstrapTimePickerWidget().first().remove();
        }
    }
    ...
    function configTimerPicker() {
        ...
        $timepicker.on('keydown', function (e) {
            switch (e.keyCode) {
                case 9: {
                    ...
                    getBootstrapTimePickerWidget().remove();
                }
            }
        });
        ...
        $timepicker.on('keyup', function (e) {
            ...
            if (!(e.keyCode === 8 || e.keyCode === 46)) {
                ...
                var hour = timeArray[0] ? timeArray[0].toString() : timeArray.toString();
                var minute = timeArray[1] ? timeArray[1].toString() : '';
    ...
    function removeWidgetWhenClickedOutside() {
        $(document).on('click', function (e) {
            if (getBootstrapTimePickerWidget().size() >= 1 && !$(e.target).attr('class').includes('sgl-timer-picker')) {
                getBootstrapTimePickerWidget().first().remove();
            }
        });
    }

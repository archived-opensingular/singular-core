 ...
 * {@link SDocument#setAttachmentPersistenceTemporaryHandler(RefService)}
...
class AttachmentContainer extends BSContainer {
    ...
    private DownloadBehavior downloader;
    ...
    public AttachmentContainer(IModel<? extends SIAttachment> model) {
        ...
        this.add(this.downloader = new DownloadBehavior(model.getObject()));
        setup(field(), model);
    }
...
//        appendScriptContainer(field.getMarkupId(), buttonContainer);
    ...
    private void appendScriptContainer(String fieldId, BSContainer buttonContainer) {
        TemplatePanel scriptContainer = (TemplatePanel)
                buttonContainer.newComponent(id -> new TemplatePanel(id,
                        () -> "<script > "+ generateInitScript(fieldId)+ " </script>\n"));
    ...
    public void renderHead(IHeaderResponse response) {
        ...
        response.render(OnDomReadyHeaderItem.forScript(generateInitScript(field().getMarkupId())));
    }

    private String generateInitScript(String fieldId) {
        return "$(function () {"

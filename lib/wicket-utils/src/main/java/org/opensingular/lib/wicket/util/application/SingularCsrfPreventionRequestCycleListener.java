 ...
 * </ul>
 */
public class SingularCsrfPreventionRequestCycleListener extends AbstractRequestCycleListener implements IRequestCycleListener, Loggable {
    private static final String ORIGIN  = "Origin";
    private static final String REFERER = "Referer";


    public SingularCsrfPreventionRequestCycleListener() {
     ...
     *
     * @param action the alternate action
     * @return this (for chaining)
     */
    public SingularCsrfPreventionRequestCycleListener setNoOriginAction(CsrfAction action) {
     ...
     * @return this
     */
    public SingularCsrfPreventionRequestCycleListener setConflictingOriginAction(CsrfAction action) {
     ...
     *
     * @param errorCode the alternate HTTP error code, default {@code 400 BAD REQUEST}
     * @return this
     */
    public SingularCsrfPreventionRequestCycleListener setErrorCode(int errorCode) {
     ...
     *
     * @param errorMessage the alternate message
     * @return this
     */
    public SingularCsrfPreventionRequestCycleListener setErrorMessage(String errorMessage) {
     ...
     *
     * @param acceptedOrigin the acceptable origin
     * @return this
     */
    public SingularCsrfPreventionRequestCycleListener addAcceptedOrigin(String acceptedOrigin) {
     ...
     * CSRF attack.
     *
     * @param targetedPage the page that is the target for the action
     ...
     *
     * @param handler the handler that is currently processing
    ...
    public void onRequestHandlerResolved(RequestCycle cycle, IRequestHandler handler) {
        if (!isEnabled()) {
            getLogger().trace("CSRF listener is disabled, no checks performed");
            return;
        }

        handler = unwrap(handler);//NOSONAR
        ...
        if (isChecked(handler)) {
            ...
            if (isChecked(targetedPage)) {
                ...
                getLogger().debug("Targeted page {} was opted out of the CSRF origin checks, allowed",
     ...
     * {@code page}.
     *
     * @param request   the current container request
     * @param sourceUri the source URI
     * @param page      the page that is the target of the request
     */
    protected void checkRequest(HttpServletRequest request, String sourceUri, IRequestablePage page) {
        if (sourceUri == null || sourceUri.isEmpty()) {
            getLogger().debug("Source URI not present in request, {}", noOriginAction);
        ...
        sourceUri = sourceUri.toLowerCase();//NOSONAR
        ...
        if (!isLocalOrigin(request, sourceUri)) {
            getLogger().debug("Source URI conflicts with request origin, {}", conflictingOriginAction);
     ...
     *
     * @param sourceUri the contents of the {@code Origin} or {@code Referer} HTTP header
        ...
        try {
            ...
            for (String whitelistedOrigin : acceptedOrigins) {
                if (sourceHost.equalsIgnoreCase(whitelistedOrigin) ||
                    ...
                    getLogger().trace("Origin {} matched whitelisted origin {}, request accepted",
                            sourceUri, whitelistedOrigin);
                    return true;
                }
            }
        } catch (URISyntaxException e) {
            getLogger().debug("Origin: {} not parseable as an URI. Whitelisted-origin check skipped.", sourceUri);
     ...
     *
     * @param uri the contents of the Origin or Referer HTTP header
    ...
    protected final String normalizeUri(String uri) {
        ...
        try {
            ...
            boolean isAlternateHttpPort  = "http".equals(scheme) && port != 80;//NOSONAR
            boolean isAlternateHttpsPort = "https".equals(scheme) && port != 443;//NOSONAR
    ...
    protected final String getTargetUriFromRequest(HttpServletRequest request) {
        ...
        if ("http".equals(scheme) && port != 80 || "https".equals(scheme) && port != 443)//NOSONAR
        {
            target.append(':');
            target.append(port);
        }
     ...
     * @param origin  the contents of the {@code Origin} HTTP header
     * @param page    the page that is targeted with this request
     */
    protected final void whitelistedHandler(HttpServletRequest request, String origin,
                                            IRequestablePage page) {
        onWhitelisted(request, origin, page);
        if (getLogger().isDebugEnabled()) {
            getLogger().debug("CSRF Origin {} was whitelisted, allowed for page {}", origin,
    ...
    protected final void matchingOrigin(HttpServletRequest request, String origin,
        ...
        if (getLogger().isDebugEnabled()) {
            getLogger().debug("CSRF Origin {} matched requested resource, allowed for page {}", origin,
                    page.getClass().getName());
        }
    }
     ...
     *
     * @param request the request
     * @param origin  the contents of the {@code Origin} HTTP header, may be {@code null} or empty
     * @param page    the page that is targeted with this request
     */
    protected final void allowHandler(HttpServletRequest request, String origin,
                                      IRequestablePage page) {
        onAllowed(request, origin, page);
        getLogger().info("Possible CSRF attack, request URL: {}, Origin: {}, action: allowed",
    ...
    protected final void suppressHandler(HttpServletRequest request, String origin,
        ...
        getLogger().info("Possible CSRF attack, request URL: {}, Origin: {}, action: suppressed",
                request.getRequestURL(), origin);

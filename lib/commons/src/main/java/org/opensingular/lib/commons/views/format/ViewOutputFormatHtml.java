...
public class ViewOutputFormatHtml extends ViewOutputFormatExportable {
    ...
    public void generateFile(@Nonnull File destination, @Nonnull ViewGenerator viewGenerator) throws IOException {
        try(FullPageHtmlGenerator page = new FullPageHtmlGenerator(destination)) {
            page.writeBegin();
            ViewOutputHtmlWriterWrap vOut = new ViewOutputHtmlWriterWrap(page.getOut(), false);
            viewGenerator.generateView(vOut);
            page.writeEndAndClose();
        }

...
public class GeradorDiagramaProcessoMBPMJGraph implements GeradorBPMN {
    ...
    private static GeradorDiagramaProcessoMBPMJGraph instance = null;

    private GeradorDiagramaProcessoMBPMJGraph() {
        /* CONSTRUTOR VAZIO */
    }
    ...
    public byte[] gerarBPMNImage(ProcessDefinition<?> definicao) {
        return gerarDiagramaImpl(definicao);
    }

    private static byte[] gerarDiagramaImpl(ProcessDefinition<?> definicao) {
        final mxGraph graph = gerarGraph(definicao);
    ...
    private static void addStyleIcone(mxStylesheet foo, String nomeEstilo, String nomeImagem) {
        ...
        def.put(mxConstants.STYLE_IMAGE, "/br/net/mirante/mbpm/view/images/" + nomeImagem);
        // def.put(mxConstants.STYLE_SPACING_TOP, mxConstants.SHAPE_IMAGE);
        foo.putCellStyle(nomeEstilo, def);
    }

    private static mxGraph gerarGraph(ProcessDefinition<?> definicao) {
    ...
    private static void addTransicaoInicio(mxGraph graph, MTask<?> taskInicial, Map<String, Object> mapaVertice) {
    ...
    private static void criarTransicao(mxGraph graph, MTransition transicao, Map<String, Object> mapaVertice) {
